...
import ims.crawler.fetch.special.FetchOutsideReplyContent;
...
public class BasicBlogFetchContentThread implements
	...
	public BasicBlogFetchContentThread(FetchParame fetchParame,
	...
	public List<Map<String, Object>> getReplyEachArticle(Element elePostDiv,
		...
		if (fetchParame.getReplyDivQuery().equals("special")) {

			FetchOutsideReplyContent fetchOutsideReplyContent = new FetchOutsideReplyContent();
			eachArticleReplyMaps = fetchOutsideReplyContent
					.getOutsideReplyContent(this.site, articlePageUrl,
							this.grabUserParame);
	...
	public List<Map<String, Object>> getArticleEachPage(String pageUrl) {
		...
		for (Element eleArticleDiv : elesArticleDiv) {
			...
			// 获取回复模块（通过调用单独的方法实现）
			List<Map<String, Object>> eachArticleReplyMaps = getReplyEachArticle(
					eleArticleDiv, pageUrl);
	...
	public Map<String, Object> call() throws Exception {
		...
		for (Post post : getPosts()) {
			...
			// 初始化第一页的document
			this.docPostFirstPage = BasicJsoupDocumentUtil.getDocument(post
					.getPostUrl());
